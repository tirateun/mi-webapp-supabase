// AgreementsForm.tsx (versión profesional corregida)
...
export default function AgreementsForm({ existingAgreement, onSave, onCancel, }: AgreementsFormProps) {
  ...
  const [selectedInternals, setSelectedInternals] = useState<Option[]>(() => {
    // Si existingAgreement ya trae responsables como array de ids, los convertimos a opciones
    const initial = existingAgreement?.internal_responsibles || existingAgreement?.internals || [];
    if (!Array.isArray(initial) || initial.length === 0) return [];
    // Si vienen como objetos {id, full_name} o solo ids, normalizamos a Option
  ...
  const [resolucion, setResolucion] = useState<string>(existingAgreement?.["Resolución Rectoral"] || "");
  ...
  const [subTipoDocente, setSubTipoDocente] = useState<string>(existingAgreement?.sub_tipo_docente || "");
  ...
  const [areasSeleccionadas, setAreasSeleccionadas] = useState<string[]>(existingAgreement?.areasSeleccionadas || []);
  ...
  // ----------------------------------
  useEffect(() => {
    // cargamos las listas que no dependen del agreement
    fetchPaises();
    fetchAreas();
    fetchResponsables();
  ...
  // Una vez que 'internos' está cargado, sincronizamos los selectedInternals si hay un existingAgreement
  useEffect(() => {
    if (!existingAgreement?.id) return;
    if (internos.length === 0) return; // esperamos a que la lista de internos llegue
  ...
  const fetchResponsablesInternos = async (agreementId: string) => {
    try {
      ...
      // convertimos los ids a opciones usando la lista 'internos' ya cargada
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existingAgreement?.id) {
        ...
        const { data, error } = await supabase.from("agreements").insert([dataToSave]).select("id").single();
        if (error) throw error;
        agreementId = data?.id;
      }
